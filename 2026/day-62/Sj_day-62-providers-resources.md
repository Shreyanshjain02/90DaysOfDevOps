...
<img width="569" height="68" alt="image" src="https://github.com/user-attachments/assets/71095e55-9c66-4894-832d-047d31ec0ba3" />
...
~> means latest version of 5.0 like in our case it is 5.100.0
>= means latest version greater than 5.0

<img width="580" height="85" alt="image" src="https://github.com/user-attachments/assets/3c35df36-603b-4555-8a4c-554ff75234b9" />

= mean specific version.
...
<img width="355" height="45" alt="image" src="https://github.com/user-attachments/assets/974fb4ac-c1a9-404e-80c2-7d76fd0931df" />

**Verify:** Apply and check the AWS VPC console. Can you see all five resources connected?

<img width="917" height="187" alt="image" src="https://github.com/user-attachments/assets/d76e85a9-5c3e-400b-9b79-01f2b5cd2faa" />
...
By default terraform executes all resources in parallel since we are using vpc.id with other resources, it create a dependecy graph to decide execution order. by this technique terraform know to create VPC first then subnet.
Execution option
Default: Parallel (Fastest).
Requirement: Sequential (Only when a dependency exists).
  -Implicit dependency (automatic way):- as we mentioned vpc.id to be used with other resources
  -explicit dependency (manual way):- we can also mention depends_on in any resource to create dependency. 

  
- What would happen if you tried to create the subnet before the VPC existed?

as we have created implicit dependency on VPC.ID it will wait till vpc is created.

- Find all implicit dependencies in your config and list them

  In above task implicit dependencies follow like:-

 - VPC - No dependency
 - subnet - depends on vpc.id
 - Internet gateway - depends on vpc.id
 - route table - depends on vpc.id
     - route- depend on gateway.id
 - route table association - depends on subnet.id and route table.id
